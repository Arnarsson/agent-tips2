...
import { ScrapedContent, LearningContent, LearningExample, LearningExercise } from './types';
import * as cheerio from 'cheerio';
import { ContentSection, CodeBlock, ImageData } from './types';
 ...
 */
export function toLearningSections(content: ScrapedContent): LearningContent {
 ...
 */
export function extractExamples(content: ScrapedContent): LearningExample[] {
  ...
  switch (strategy) {
    ...
    case 1: // Replace words with less effective alternatives
      if (words.length > 3) {
        ...
        const wordToReplace = Object.keys(replacements).find(word => 
          sentence.toLowerCase().includes(word.toLowerCase())
        );
        
        if (wordToReplace) {
          return sentence.replace(
            new RegExp(wordToReplace, 'i'),
            replacements[wordToReplace]
          );
        }
      }
      break;
 ...
 */
export function transformHuggingFaceContent(html: string, url: string): ScrapedContent {
  ...
  const allProcessedParagraphs: string[] = []; // Track all paragraphs across sections for cross-section duplication
  ...
  let topics: string[] = $('.tags a, .category-tags a, .doc-tag').map((_, el) => $(el).text().trim()).get();
 ...
 */
function isDuplicateParagraph(existingParagraphs: string[], newParagraph: string): boolean {
 ...
 */
function isDuplicateCodeBlock(existingBlocks: CodeBlock[], newCode: string): boolean {
 ...
 */
function extractTopicsFromContent(content: ContentSection[], title: string, url: string): string[] {
  const allText = title + ' ' + 
  ...
  if (url.includes('tutorial')) extractedTopics.add('tutorials');
  if (url.includes('classif')) extractedTopics.add('classification');
  if (url.includes('prompt')) extractedTopics.add('prompting');
  if (url.includes('task')) extractedTopics.add('tasks');
 ...
 */
function determineDifficulty(content: ContentSection[]): 'beginner' | 'intermediate' | 'advanced' {
